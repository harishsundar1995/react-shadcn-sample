...
const DocsPropertiesPanel = () => {
  ...
  return (
    <aside className="w-80 bg-white border-l border-gray-200 sticky top-16 h-[calc(100vh-4rem)] overflow-y-auto">
      <div className="p-6">
        {/* Table of Contents */}
        <div className="mb-8">
          ...
          <nav className="space-y-2">
            {tocItems.map((item) => (
              <button
                ...
                className={`block w-full text-left text-sm px-2 py-1 rounded hover:bg-gray-100 transition-colors ${
          ...
          <div className="space-y-2">
            {usageItems.map((item) => (
              <button
                key={item.id}
                onClick={() => scrollToSection(item.id)}
                className={`block w-full text-left text-sm font-mono px-2 py-1 rounded hover:bg-gray-100 transition-colors ${
                  activeSection === item.id ? 'text-blue-600 bg-blue-50' : 'text-gray-600'
                }`}
              >
                {item.label}
